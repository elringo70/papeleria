...
function ticketStore() {
  ...
  const addProductToTicket = async (product) => {
    update(tickets => {
      ...
      if (tickets[ticketPosition].products.length > 0) {
        tickets[ticketPosition].products.forEach((ticketProduct, index) => {
          console.log(ticketProduct.product._id === product._id)
          if (ticketProduct.product._id === product._id) {
            tickets[ticketPosition].products[index].quantity++
          } else {
            tickets[ticketPosition].products.push({ product: product, quantity: 1 })
          }
        });
      } else {
        tickets[ticketPosition].products.push({ product, quantity: 1 })
      }
